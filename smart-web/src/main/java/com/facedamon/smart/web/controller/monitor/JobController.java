...
public class JobController extends BaseController {
    ...
    @Log(model = "导出定时任务",businessType = BusinessType.EXPORT)
    @RequiresPermissions("monitor:job:export")
    @PostMapping("/export")
    @ResponseBody
    public Response export(Job job){
        List<Job> jobs = jobService.selectJobList(job);
        ExcelUtils<Job> utils = new ExcelUtils<>(Job.class);
        return utils.export(jobs,"job");
    }
